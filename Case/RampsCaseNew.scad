...
module rampsCase(){
	...
	union(){
	    rotate([90,0,90])
	    difference(){
		translate([2,1,0]) linear_extrude(height=lcd_x+thick*2,center=true)  
                scale([1.1,1.1]) rampsSideShape();
...
/*	    // a tail like hole for extrutor wires
	...
	// 40x40 fans mounting hole
	//for(i=[-40,40])
	translate([-12,42,-30])rotate([-90,0,0])union(){
	    //cube([36,36,8],center=true);
	    for(i=[-16,16])for(j=[-16,16])translate([i,j,0])
	    cylinder(r=m3ScrewRadius, h=20, center=true, $fn=20);
	    cylinder(d=38,h=10,center=true,$fn=20);
...
*/
        $fn=20;
        translate([13,44,-42]) rotate([90,0,0])circuitCoverProfile();

	// USB connector mount
	translate([-47,35,-30])rotate([0,90,0])
	{
...
module circuitCoverProfile(){
    union(){
        ...
        translate([4,3,-14]) cylinder(d=3,h=15);
        translate([4, 3+18.5, -14]) cylinder(d=3, h=15);
        translate([4 + 42,3,-14]) cylinder(d=3,h=15);
        translate([4 + 42, 3+18.5, -14]) cylinder(d=3, h=15);
