...
float probe_bed(float x, float y)
{
  ...
  do {
    ...
    //SERIAL_ECHO(" ");
  } while ((probe_z != probe_bed_z) and (probe_count < 21)); // The succeed two probe return same result then stop
  //SERIAL_ECHOLN("");
  /*
    if (probe_count > 2)
    {
	  ...
	  if (code_seen('A')) 
	    {
	      ...
	      // already done, no need to do again
	      /*
	      for (int y = 0; y <=6; y++)
		{
		  for (int x = 0; x <=6; x++)
		...
		if ((bed_level_c > 3) or (bed_level_c < -3))
		  {
		    ...
		    SERIAL_ECHOLN("Build height is not set correctly..");
